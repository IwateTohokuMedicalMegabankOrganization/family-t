...
var personal_information = {"id":"606cb77e-d364-476c-bcc0-55c5c3c8bfc9","name":"fefe","gender":"MALE","date_of_birth":"12/29/1891","twin_status":"NO","height":"","height_unit":"","weight":"","weight_unit":"pound","Health History":[],"race":{"American Indian or Alaska Native":false,"Asian":false,"Black or African-American":false,"Native Hawaiian or Other Pacific Islander":false,"White":false,"Asian Indian":false,"Chinese":false,"Filipino":false,"Japanese":false,"Korean":false,"Vietnamese":false,"Other Asian":false,"Unknown Asian":false,"Chamorro":false,"Guamanian":false,"Native Hawaiian":false,"Samoan":false,"Unknown South Pacific Islander":false},"ethnicity":{"Hispanic or Latino":false,"Ashkenazi Jewish":false,"Not Hispanic or Latino":false,"Central American":false,"Cuban":false,"Dominican":false,"Mexican":false,"Other Hispanic":false,"Puerto Rican":false,"South American":false},"father":{"gender":"MALE","id":"e84ada91-9c4e-4406-b759-38b77837a892","Health History":[],"relationship":"father"},"mother":{"gender":"FEMALE","id":"a8427f11-44da-4222-87e0-efa81ab570af","Health History":[],"relationship":"mother"},"maternal_grandfather":{"gender":"MALE","id":"5ccbdc66-dc65-4514-923f-0d595ca13650","Health History":[],"relationship":"maternal_grandfather"},"maternal_grandmother":{"gender":"FEMALE","id":"65c09f2f-0a99-4b36-ae68-b92e3916227a","Health History":[],"relationship":"maternal_grandmother"},"paternal_grandfather":{"gender":"MALE","id":"2208e1b7-e184-495b-9303-f7f03a6f5842","Health History":[],"relationship":"paternal_grandfather"},"paternal_grandmother":{"gender":"FEMALE","id":"0ee1ae09-ec9f-4a3e-958b-e4fa78bae15a","Health History":[],"relationship":"paternal_grandmother"}};
...
function calcSuitaScore(){
	...
	// 各種計算
	$('#age').val(calculateAgeScore()).change();
	$('#gender').val(calculateGenderScore()).change();
	$('#isSmoker').val(calculateSmokerScore()).change();
	$('#hdlValue').val(calculateHdlScore()).change();
	$('#ldlValue').val(calculateLdlScore()).change();
	$('#bloodPressure').val(calculateBloodPressureScore()).change();
	$('#suger').val(calculateSugerScore()).change();
	$('#eight').val(calculateEightScore()).change();
...
function load_dl(){
	clientValue.hdlValue = $('#input_hdl').val();
	clientValue.ldlValue = $('#input_ldl').val();
}

function load_suger(){
	clientValue.suger = $('#input_suger').prop('checked');
}

function load_bp(){
	clientValue.bloodPressureMax = $('#input_bpmax').val();
	clientValue.bloodPressureMin = $('#input_bpmin').val();
}

function load_smoker(){
	clientValue.isSmoker = $('#input_issmoker').prop('checked');
}

function calculateEightScore() {
	var eight = clientValue.eight;
...
function calculateSugerScore() {
	var suger = clientValue.suger;
...
function calculateBloodPressureScore() {
	var bpMax = clientValue.bloodPressureMax;
	var bpMin = clientValue.bloodPressureMin;

	if( 160  <= bpMax && bpMax <= 179 ) return 6;
	if( 100  <= bpMin && bpMin <= 109 ) return 6;

	if( 140 <= bpMax && bpMax <= 159 ) return 4;
	if( 90  <= bpMin && bpMin <= 99 ) return 4;

	if(130 <= bpMax && bpMax <= 139 ) return 0;
	if(85  <= bpMin && bpMin <= 89 ) return 0;

	if( 120 <= bpMax && bpMax <= 129 ) return 0;
	if( 80  <= bpMin && bpMin <= 84 ) return 0;

	if( bpMax < 120 && bpMin < 80 ) return -7;

	return 0;
}


function calculateLdlScore() {
	var ldl = clientValue.ldlValue;

	if( ldl < 100 ) return 0;
	if( 100 <= ldl && ldl <= 139 ) return 5;
	if( 140 <= ldl && ldl <= 159 ) return 7;
	if( 160 <= ldl && ldl <= 179 ) return 10;
	if( 180 <= ldl ) return 11;
...
function calculateHdlScore() {
	var hdl = clientValue.hdlValue;

	if( hdl < 40 ) return 0;
	if( 40 <= hdl && hdl <= 59 ) return 5;
	if( 60 <= hdl ) return -6;
...
function calculateSmokerScore() {
	var isSmoker = clientValue.isSmoker;
...
function calculate_score() {
	...
	score.total = 
		...
		+ parseInt(score.isSmoker) 
		+ parseInt(score.hdlValue)
		+ parseInt(score.ldlValue) 
		+ parseInt(score.bloodPressure)
		+ parseInt(score.suger)
		+ parseInt(score.eight);
...
function updateScore() {
	score.age				= $('#age').val();
	score.gender			= $('#gender').val();
	score.isSmoker			= $('#isSmoker').val();
	score.hdlValue			= $('#hdlValue').val();
	score.ldlValue			= $('#ldlValue').val();
	score.bloodPressure		= $('#bloodPressure').val();
	score.suger				= $('#suger').val();
	score.eight				= $('#eight').val();

	if(  score.age .length == 0 ) score.age = 0;
	if(  score.gender .length == 0 ) score.gender = 0;
	if(  score.isSmoker .length == 0 ) score.isSmoker = 0;
	if(  score.hdlValue .length == 0 ) score.hdlValue = 0;
	if(  score.ldlValue .length == 0 ) score.ldlValue = 0;
	if(  score.bloodPressure .length == 0 ) score.bloodPressure = 0;
	if(  score.suger .length == 0 ) score.suger = 0;
	if(  score.eight .length == 0 ) score.eight = 0;
