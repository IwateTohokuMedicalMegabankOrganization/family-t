...
var personal_information = {
  "id": "68706c56-4402-4fde-a75b-1923116f509c",
  "name": "titi",
  "gender": "MALE",
  "date_of_birth": "07/22/1981",
  "twin_status": "NO",
  "height": "",
  "height_unit": "",
  "weight": "",
  ...
  "father": {
    "gender": "MALE",
    "id": "7e4d0e41-ed6d-403d-acea-095a51d059fa",
    "Health History": [
      {
        "Disease Name": "Cancer",
        "Detailed Disease Name": "胃がん",
        "Age At Diagnosis": "thirties",
        "Disease Code": "SNOMED_CT-363349007"
      }
    ],
    "relationship": "father",
    "name": "",
  ...
  "mother": {
    "gender": "FEMALE",
    "id": "89709d8b-c512-4105-8fd1-e8d6b5b9bd32",
    "Health History": [],
    "relationship": "mother"
  },
  "maternal_grandfather": {
    "gender": "MALE",
    "id": "83a6e584-e946-49f8-8bcd-b807dc40ab32",
    "Health History": [],
    "relationship": "maternal_grandfather"
  },
  "maternal_grandmother": {
    "gender": "FEMALE",
    "id": "64486f2f-916b-4ce3-9a2c-cca910d65ce6",
    "Health History": [],
    "relationship": "maternal_grandmother"
  },
  "paternal_grandfather": {
    "gender": "MALE",
    "id": "5c4df2dc-be5b-4cd3-869e-7f434ebc9d2a",
    "Health History": [],
    "relationship": "paternal_grandfather"
  },
  "paternal_grandmother": {
    "gender": "FEMALE",
    "id": "ee7cdec6-7a95-4149-852c-6481dd1af988",
    "Health History": [],
    "relationship": "paternal_grandmother"
  },
  "brother_0": {
    "gender": "MALE",
    "id": "998464b8-e032-43eb-80a5-2abea406e493",
    "Health History": [],
...
$(document).ready(function() {
	...
	$("#gastric_content").hide();
	$("#extra_info_dialog").show();
	
	$("#gastricNextButton").on('click',function(){
		calcScore();
		$("#extra_info_dialog").hide();
		$("#gastric_content").show();
	});
});
...
function calcScore(){
	...
	$('#abcType').val(calculateAbcTypeScore()).change();
	$('#isSmoker').val(calculateSmokerScore()).change();
	$('#solt').val(calculateSoltScore()).change();
...
function load_helico(){
	clientValue.ishelico = $('#input_ishelico').prop('checked');
}
function load_chronic(){
	clientValue.ischronic = $('#input_ischronic').prop('checked');
}
function load_solt(){
	clientValue.solt = $('#input_solt').prop('checked');
}
function load_smoker(){
	clientValue.isSmoker = $('#input_issmoker').prop('checked');
}
function calculateAbcTypeScore() {
	var ishelico = clientValue.ishelico;
	var ischronic = clientValue.ischronic;

	if( !ischronic && !ischronic ) return 0;
	if( ischronic && !ischronic ) return 8;
	if( ischronic && ischronic ) return 11;
	if( !ischronic && ischronic ) return 11;
	...
	if (!(personal_information.adopted == true || personal_information.adopted == "true") ) {
	    ...
	    if(temp == 'father' || temp == 'mother' || temp == 'brothe' || temp == 'sister') {
	      if (!(item.adopted == true) &&  item['Health History']) {
	          ...
	          if (item['Health History'][i]['Disease Code'] == "SNOMED_CT-363349007") inherited = true;
...
function calculateSoltScore() {
	var solt = clientValue.solt;
...
function calculateAgeScore() {
	...
	if( gender == "FEMALE" ){
		...
		if( 45 <= age && age <= 49 ) return 1;
		if( 50 <= age && age <= 54 ) return 2;
		if( 55 <= age && age <= 59 ) return 3;
		if( 60 <= age && age <= 64 ) return 4;
		if( 65 <= age && age <= 70 ) return 5;
		if( 71 <= age ) return 5;
	}
	
	if( age < 40 ) return 0;
	if( 40 <= age && age <= 44 ) return 1;
	if( 45 <= age && age <= 49 ) return 3;
	if( 50 <= age && age <= 54 ) return 4;
	if( 55 <= age && age <= 59 ) return 6;
	if( 60 <= age && age <= 64 ) return 8;
	if( 65 <= age && age <= 70 ) return 10;
	if( 71 <= age ) return 10;

	return 0;
}

function calculateProbability10Score(){

	if( 10 >= score.total ) return "≦0.4%";
	if( 11 == score.total ) return "0.6%";
	if( 12 == score.total ) return "0.7%";
	if( 13 == score.total ) return "0.9%";
	if( 14 == score.total ) return "1.2%";
	if( 15 == score.total ) return "1.5%";
	if( 16 == score.total ) return "1.9%";
	if( 17 == score.total ) return "2.5%";
	if( 18 == score.total ) return "3.2%";
	if( 19 == score.total ) return "4.1%";
	if( 20 == score.total ) return "5.2%";
	if( 21 == score.total ) return "6.6%";
	if( 22 == score.total ) return "8.3%%";
	if( 23 == score.total ) return "10.6%";
	if( 24 == score.total ) return "13.4%";
...
function calculate_score() {
	...
	$('#probability10').attr('value',calculateProbability10Score());
